                                ...
                                while ($row = $result->fetch_assoc()){
                                    if ($_SERVER["REQUEST_METHOD"] == "GET") {
                                        if($authkeyBank == $sltl){ 							                                                                //als key van bank is
                                            if($row['pogingen'] < 3){ 									                                                //als nieuw balans nog niet het huidige balans is
                                                $sql        =   "UPDATE pincodes SET `pogingen` = 5 WHERE rekeningnummer= '$mgrkn'"; 	                    //zet nieuw balans
                                                $conn->query($sql); 									                                                    //voer sql uit      
                                                echo "1";										                                                            //geef 1 terug, balans is aangepast
                                            } else if($row['pogingen'] >= 3){
